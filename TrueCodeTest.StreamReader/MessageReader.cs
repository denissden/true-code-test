...
public class MessageReader(byte delimiter, int bufferSize = 8192, bool lastChunkIsMessage = true, bool allowEmptyMessages = false)
{
    ...
    public async IAsyncEnumerable<byte[]> ReadMessagesAsync(Stream stream, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (messageBuffer.Count > 0 && lastChunkIsMessage)
        {
            yield return messageBuffer.ToArray();
        }
    }
