...
builder.Services.AddScoped<Hub>(_ => Hub.Connect(connectionFactory: connFactory));
...
app.MapGet("/fibonacci", async ([FromQuery] int f, [FromQuery] int timeout, [FromServices] Hub hub) =>
    { 
        ...
        _ = Task.Delay(timeout).ContinueWith(async task =>
        {
...
record FibonacciResponse(int F, int N, long TotalMilliseconds);